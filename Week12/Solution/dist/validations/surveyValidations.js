...
exports.updateSurveySchema = zod_1.z.object({
    title: zod_1.z.string().min(1, "Title cannot be empty").max(100, "Title must be 100 characters or less").optional(),
    questions: zod_1.z.array(zod_1.z.object({
        id: zod_1.z.number().int().positive().optional(),
        text: zod_1.z.string().min(1, "Question text cannot be empty").max(500, "Question text must be 500 characters or less"),
        options: zod_1.z.array(zod_1.z.object({
            id: zod_1.z.number().int().positive().optional(),
            text: zod_1.z.string().min(1, "Option text cannot be empty").max(200, "Option text must be 200 characters or less"),
        })).min(2, "At least two options are required").max(10, "Maximum of 10 options allowed"),
    })).min(1, "At least one question is required").max(50, "Maximum of 50 questions allowed"),
});
